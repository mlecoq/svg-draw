...
import {DrawItem, DrawItemType, DrawCoreProps, hslColor, Size} from './types';
...
>(({drawingMode, image, linearGradient}, ref) => {
  ...
  >({
    onStart: ({x, y}, ctx) => {
      ...
      ctx.initialItem = currentItem.value;
      ...
      switch (currentItem.value?.type) {
        ...
        case 'singleHead':
          ...
          if (
            ...
            pDistance({x: startX, y: startY}, {x1, x2, y1, y2}) <= THRESHOLD &&
    ...
    onActive: (
      ...
      {startX, startY, initialItem, zone},
    ) => {
      switch (currentItem.value?.type) {
        case 'pen':
          if (
            initialItem?.type === currentItem.value.type &&
            zone === 'CENTER'
          ) {
            currentItem.value = {
              type: 'pen',
          ...
          break;
        case 'ellipse':
          if (initialItem?.type === currentItem.value.type) {
            const rx =
              typeof initialItem.data.rx === 'string'
                ? parseFloat(initialItem.data.rx)
                : initialItem.data.rx || 0;

            const ry =
              typeof initialItem.data.ry === 'string'
                ? parseFloat(initialItem.data.ry)
                : initialItem.data.ry || 0;

            const cx =
              typeof initialItem.data.cx === 'string'
                ? parseFloat(initialItem.data.cx)
                : initialItem.data.cx || 0;

            const cy =
              typeof initialItem.data.cy === 'string'
                ? parseFloat(initialItem.data.cy)
                : initialItem.data.cy || 0;
        ...
        case 'rectangle':
          if (initialItem?.type === currentItem.value.type) {
            const height =
              typeof initialItem.data.height === 'string'
                ? parseFloat(initialItem.data.height)
                : initialItem.data.height || 0;

            const width =
              typeof initialItem.data.width === 'string'
                ? parseFloat(initialItem.data.width)
                : initialItem.data.width || 0;

            const x =
              typeof initialItem.data.x === 'string'
                ? parseFloat(initialItem.data.x)
                : initialItem.data.x || 0;

            const y =
              typeof initialItem.data.y === 'string'
                ? parseFloat(initialItem.data.y)
                : initialItem.data.y || 0;
        ...
        case 'doubleHead':
          if (initialItem?.type === currentItem.value.type) {
            const x1 =
              typeof initialItem.data.x1 === 'string'
                ? parseFloat(initialItem.data.x1)
                : initialItem.data.x1 || 0;

            const y1 =
              typeof initialItem.data.y1 === 'string'
                ? parseFloat(initialItem.data.y1)
                : initialItem.data.y1 || 0;

            const x2 =
              typeof initialItem.data.x2 === 'string'
                ? parseFloat(initialItem.data.x2)
                : initialItem.data.x2 || 0;

            const y2 =
              typeof initialItem.data.y2 === 'string'
                ? parseFloat(initialItem.data.y2)
                : initialItem.data.y2 || 0;
  ...
  useEffect(() => {
    const subDidShow = Keyboard.addListener(
      'keyboardDidShow',
      _keyboardDidShow,
    );
    const sudDidHide = Keyboard.addListener(
      'keyboardDidHide',
      _keyboardDidHide,
    );
  ...
  const onPressItem = useCallback(
    ...
    [color, currentItem, strokeWidth, updateDoneItems],
  );
